...
#import "UIColor+xh_utils.h"
#import "XHDBManager.h"
@interface XHColorSelectedView ()<UICollectionViewDelegate,UICollectionViewDataSource>
{
    NSInteger _maxColorNumber;
}
@property (weak, nonatomic) IBOutlet UIView *contentView;
@property (nonatomic, strong) XHColorSelectedViewResultCallBack resultCallBack;
...
@end
@implementation XHColorSelectedView
+ (instancetype)defaultView{
    static XHColorSelectedView * colorSelectedView = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        colorSelectedView = [XHColorSelectedView instanceFromNib];
    });
...
- (IBAction)enterButtonClicked:(UIButton *)sender {
    ...
    BOOL flag = [[XHDBManager defaultManager] existData:colorInfo];
    if (flag == NO) {
        if ([self.colorDataSource count] >= _maxColorNumber) {
            ColorInfo *firstColorInfo = [self.colorDataSource firstObject];
            [[XHDBManager defaultManager] deleteData:firstColorInfo result:^(BOOL flag) {
                [self.colorDataSource removeObjectAtIndex:0];
                [self.collectionView reloadData];
                [[XHDBManager defaultManager] insertData:colorInfo result:^(BOOL flag) {
...
- (void)showInView:(UIView*)view result:(XHColorSelectedViewResultCallBack) result{
    ...
    [self layoutIfNeeded];
    ColorInfo *colorInfo = [ColorInfo new];
    [[XHDBManager defaultManager] selectData:colorInfo result:^(NSMutableArray *resultSet) {
