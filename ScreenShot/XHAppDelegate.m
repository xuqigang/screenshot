...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    [[XHDBManager defaultManager] registerTableClass:@[@"ColorInfo"] result:^(NSDictionary *response) {
        NSLog(@"%@",response);
    }];
    [SVProgressHUD setMaximumDismissTimeInterval:2.5];
    [SVProgressHUD setMinimumDismissTimeInterval:2.5];
    // Override point for customization after application launch.
    XHHomeVC *vc = [XHHomeVC instanceFromNib];
    XHBaseNC * nv = [[XHBaseNC alloc] initWithRootViewController:vc];
