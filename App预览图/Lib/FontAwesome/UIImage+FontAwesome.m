...
+(UIImage*)imageWithIdentifier:(NSString*)identifier iconColor:(UIColor*)iconColor size:(NSInteger)size {
    if (!iconColor) {
        iconColor = [UIColor whiteColor];
    }
    ...
    //// Content Edge Insets
    CGSize imageSize = [textContent sizeWithAttributes:attributes];
    imageSize = CGSizeMake(imageSize.width * 1.1, imageSize.height * 1.05);
    
    CGRect textRect = CGRectZero;
    textRect.size = imageSize;
    
    CGPoint origin = CGPointMake(imageSize.width * 0.05, imageSize.height * 0.025);
